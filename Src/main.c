...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define APP_ENABLE_DCMI_SNAPSHOT 1U
...
int main(void)
{
  ...
  while (1)
  {
    #if APP_ENABLE_DCMI_SNAPSHOT
    DCMI_Capture_Task();
    WS2812_TaskZones(g_dcmi_zone0_brightness_percent,
...
static void MX_DMA_Init(void)
{
  ...
  /* DCMI frame interrupt is enabled by the HAL after DMA completes. */
